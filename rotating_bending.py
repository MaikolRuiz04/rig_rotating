...
GPIO.setup(R_PWM, GPIO.OUT)                      # Rpwm pin enable
GPIO.setup(L_PWM, GPIO.OUT)                      # Lpwm pin enable

# setup Hall sensor pin
GPIO.setup(HALL_PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)

# Forward Drive PWM setup
p1 = GPIO.PWM(R_PWM,5000)                    # r_pwm,  freq = 20kHz
p1.start(0)                                   # start at 0 duty cycle

# Reverse Drive PWM setup (same as above)
duty1 = 0                                     # keep track of duty cycle
p2 = GPIO.PWM(L_PWM,5000)
p2.start(0)
...
def motor1(duty):
    ...
    if duty > 0:                            # Positive duty cycle -> forward drive (clockwise?)
        p2.ChangeDutyCycle(0)               # just in case, set reverse drive to 0 duty cycle
        if duty > duty1:
            while duty > temp:
                temp = temp + 1
                p1.changeDutyCycle(abs(temp))
        else: 
            while duty < temp:
                temp = temp-1
                p1.ChangeDutyCycle(abs(temp))
        p1.ChangeDutyCycle(abs(duty))           # change duty cycle    else 
    else:                                   # negative duty cycle -> reverse drive (counterclockwise?)
        p1.ChangeDutyCycle(0)               # just in case, set forward drive to 0 duty cycle
        if duty > duty1:
            while duty > temp:
                temp = temp + 1
                p2.changeDutyCycle(abs(temp))
        else: 
            while duty < temp:
                temp = temp-1
                p2.ChangeDutyCycle(abs(temp))
        p2.ChangeDutyCycle(abs(duty))           # change duty cycle    else 
...
GPIO.add_event_detect(HALL_PIN, GPIO.FALLING, callback=hall_callback, bouncetime=10)
